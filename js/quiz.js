// ============================================================
// FILE: js/quiz.js
// PURPOSE: Full quiz engine — handles question flow, batch
//          generation, AI API calls, progress tracking,
//          cache/resume logic, and final submission
// DEPENDS ON: config.js (API keys), api/chat.js (serverless)
// USED BY: quiz.html
// ============================================================

// ── SECTION: Hardcoded Questions (Q1–Q3) ─────────────────────
...
let conversationHistory  = [{ role: "system", content: QUESTION_SYSTEM_PROMPT }];
let allAnswers           = []; // { question, answer, extraContext, category }
let currentQuestion      = null;
let isGenerating         = false;

// ── SECTION: Cache / Resume ───────────────────────────────────
...
function restoreProgress() {
  ...
  try {
    ...
    conversationHistory  = state.conversationHistory  || [{ role: "system", content: QUESTION_SYSTEM_PROMPT }];
    return true;
  } catch (e) {
    return false;
  }
...
const optionalContainer = document.getElementById('optional-container');  // Q26 screen
const optionalInput     = document.getElementById('optional-q26-input'); // Q26 textarea
...
// ── SECTION: API Call (via Vercel function) ───────────────────
...
// ── SECTION: Fetch Next Batch of 4 Questions ─────────────────
async function fetchNextBatch() {
  ...
  return batch
    ...
    .map(q => ({ ...q, category: q.category || 'PERSONALITY' }));
}
...
// ── SECTION: Render Question ──────────────────────────────────
function renderQuestion() {
  questionText.textContent    = currentQuestion.question;
  optionsContainer.innerHTML  = '';
  optionsContainer.style.display = 'grid';

  // Remove any previous pen toggle / extra context box
  ...
  // Pen icon + expandable extra context box
  ...
  // Back button and optional step visibility
...
// ── SECTION: Handle Option Selection ─────────────────────────
async function handleSelectOption(btn, selectedOption) {
  ...
  // Grab extra context if provided
  ...
  // Append to conversation history for final analysis context
...
async function advanceStep() {
  if (currentQuestionIndex < 3) {
    ...
    if (currentBatchIndex < questionBatch.length) {
      // Serve from cached batch — no API call
      currentQuestion = questionBatch[currentBatchIndex];
      currentBatchIndex++;
      saveProgress();
      animateQuestionTransition(renderQuestion);
    } else {
      // Fetch new batch from AI
      startLoading();
...
// ── SECTION: Progress Summary Traits ─────────────────────────
...
backBtn.onclick = () => {
  ...
  if (currentQuestionIndex < 3) {
    ...
    if (currentBatchIndex > 0) currentBatchIndex--;
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (resumed && currentQuestionIndex > 0) {
    if (currentQuestionIndex < 3) {
      ...
      // Edge case — reset to fresh start
